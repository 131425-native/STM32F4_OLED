 ...
 * @retvalue   :None
******************************************************************************/  
void TP_Drow_Touch_Point(u16 x,u16 y)
{
 ...
 * @retvalue   :None
******************************************************************************/   
void TP_Draw_Big_Point(u16 x,u16 y)
{	    
...
{	  
	...
	LCD_ShowString_CH_EN(40,160,(u8*)"x1:",1);
 	LCD_ShowString_CH_EN(40+80,160,(u8*)"y1:",1);
 	LCD_ShowString_CH_EN(40,180,(u8*)"x2:",1);
 	LCD_ShowString_CH_EN(40+80,180,(u8*)"y2:",1);
	LCD_ShowString_CH_EN(40,200,(u8*)"x3:",1);
 	LCD_ShowString_CH_EN(40+80,200,(u8*)(u8*)(u8*)"y3:",1);
	LCD_ShowString_CH_EN(40,220,(u8*)(u8*)"x4:",1);
 	LCD_ShowString_CH_EN(40+80,220,(u8*)"y4:",1);  
 	LCD_ShowString_CH_EN(40,240,(u8*)"fac is:",1);  
	sprintf((char *)red,"%d",x0);
	LCD_ShowString_CH_EN(40+24,160,red,4);		//显示数值
	sprintf((char *)red,"%d",y0);
	LCD_ShowString_CH_EN(40+24+80,160,red,4);	//显示数值
	sprintf((char *)red,"%d",x1);
	LCD_ShowString_CH_EN(40+24,180,red,4);		//显示数值
	sprintf((char *)red,"%d",y1);
	LCD_ShowString_CH_EN(40+24+80,180,red,4);	//显示数值
	sprintf((char *)red,"%d",x2);
	LCD_ShowString_CH_EN(40+24,200,red,4);		//显示数值
	sprintf((char *)red,"%d",y2);
	LCD_ShowString_CH_EN(40+24+80,200,red,4);	//显示数值
	sprintf((char *)red,"%d",x3);
	LCD_ShowString_CH_EN(40+24,220,red,4);		//显示数值
	sprintf((char *)red,"%d",y3);
	LCD_ShowString_CH_EN(40+24+80,220,red,4);	//显示数值
	sprintf((char *)red,"%d",fac);
 	LCD_ShowString_CH_EN(40+56,lcddev.width,red,3); 	//显示数值,该数值必须在95~105范围之内.
}
...
{								 
	...
	LCD_Clear();//清屏   
	LCD_SetTextColor(RED);//红色 
	LCD_Clear();//清屏 	   
	LCD_SetTextColor(BLACK);
	LCD_ShowString_CH_EN(10,40,(u8*)"Please use the stylus click the",1);//显示提示信息
	LCD_ShowString_CH_EN(10,56,(u8*)"cross on the screen.The cross will",1);//显示提示信息
	LCD_ShowString_CH_EN(10,72,(u8*)"always move until the screen ",1);//显示提示信息
	LCD_ShowString_CH_EN(10,88,(u8*)"adjustment is completed.",1);//显示提示信息
	...
	while(1)//如果连续10秒钟没有按下,则自动退出
	{
		...
		{	
			...
			{			   
				case 1:						 
					TP_Drow_Touch_Point(20,20);				//清除点1 
					TP_Drow_Touch_Point(lcddev.width-20,20);	//画点2
					break;
				case 2:
 					TP_Drow_Touch_Point(lcddev.width-20,20);	//清除点2
					TP_Drow_Touch_Point(20,lcddev.height-20);	//画点3
					break;
				case 3:
 					TP_Drow_Touch_Point(20,lcddev.height-20);			//清除点3
 					TP_Drow_Touch_Point(lcddev.width-20,lcddev.height-20);	//画点4
					break;
				case 4:	 //全部四个点已经得到
	    		    //对边相等
					...
					{
						cnt=0;
 				    	TP_Drow_Touch_Point(lcddev.width-20,lcddev.height-20);	//清除点4
   	 					TP_Drow_Touch_Point(20,20);								//画点1
					...
					if(abs(tp_dev.xfac)>2||abs(tp_dev.yfac)>2)//触屏和预设的相反了.
					{
						cnt=0;
 				    	TP_Drow_Touch_Point(lcddev.width-20,lcddev.height-20);	//清除点4
   	 					TP_Drow_Touch_Point(20,20);								//画点1
						LCD_ShowString_CH_EN(40,26,(u8*)"TP Need readjust!",1);
					...
					LCD_Clear();//清屏
					LCD_ShowString_CH_EN(35,110,(u8*)"Touch Screen Adjust OK!",1);//校正完成
					delay_ms(1000);
					TP_Save_Adjdata();  
 					LCD_Clear();//清屏   
					return;//校正完成				 
...
{			    		   
	...
	//所以上拉之前,必须使能时钟.才能实现真正的上拉输出
		...
		RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOB|RCC_AHB1Periph_GPIOE | RCC_AHB1Periph_GPIOC, ENABLE);//使能GPIOB,C,F时钟

    //GPIOB1,2初始化设置
    GPIO_InitStructure.GPIO_Pin = GPIO_Pin_0;//PB1/PB2 设置为上拉输入
    ...
    GPIO_Init(GPIOE, &GPIO_InitStructure);//初始化
		...
		GPIO_InitStructure.GPIO_Pin = GPIO_Pin_8;//PB8设置为推挽输出
		GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;//输出模式
	  GPIO_Init(GPIOB, &GPIO_InitStructure);//初始化
		
		GPIO_InitStructure.GPIO_Pin = GPIO_Pin_1;//PC5设置为推挽输出
		GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;//输出模式
	  GPIO_Init(GPIOE, &GPIO_InitStructure);//初始化	
